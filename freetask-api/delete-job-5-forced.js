...
async function run() {
    ...
    // delete payments and escrow associated with this job first
    await prisma.payment.deleteMany({ where: { jobId } });
    await prisma.escrow.deleteMany({ where: { jobId } });

    // delete job
    await prisma.job.delete({ where: { id: jobId } });
    console.log('Job 5 deleted successfully along with its dependencies.');
