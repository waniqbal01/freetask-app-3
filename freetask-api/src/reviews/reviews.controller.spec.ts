  ...
  describe('create', () => {
    ...
    it('should throw 400 when revieweeId is missing', async () => {
      ...
      mockReviewsService.create.mockRejectedValue(
        new Error('revieweeId is required'),
      );
    ...
    it('should throw 403 when self-reviewing (revieweeId === reviewerId)', async () => {
      ...
      await expect(controller.create(userId, selfReviewDto)).rejects.toThrow(
        ForbiddenException,
      );
      await expect(controller.create(userId, selfReviewDto)).rejects.toThrow(
    ...
    it('should throw 403 when reviewee not part of job', async () => {
      ...
      await expect(
        controller.create(userId, invalidRevieweeDto),
      ).rejects.toThrow(ForbiddenException);
      await expect(
        controller.create(userId, invalidRevieweeDto),
      ).rejects.toThrow('Reviewee must be part of this job');
    });
    ...
    it('should throw 403 when job is not completed', async () => {
      ...
      await expect(controller.create(userId, validDto)).rejects.toThrow(
        ForbiddenException,
      );
      await expect(controller.create(userId, validDto)).rejects.toThrow(
    ...
    it('should reject negative revieweeId', async () => {
      ...
      mockReviewsService.create.mockRejectedValue(
        new Error('revieweeId must be positive'),
      );
