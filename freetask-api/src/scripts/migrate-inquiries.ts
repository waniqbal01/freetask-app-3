...
async function main() {
  ...
  for (const inquiry of inquiries) {
    if (!inquiry.clientId || !inquiry.freelancerId) {
      console.warn(
        `Skipping inquiry ${inquiry.id}: Missing client or freelancer ID`,
      );
      continue;
    }
    ...
    try {
      ...
      if (inquiry.description && inquiry.messages.length === 0) {
        ...
        if (!existing) {
          console.log(
            `Creating initial message from description for Inquiry ${inquiry.id}`,
          );
  ...
  console.log(
    `Found ${messagesWithNoConv.length} jobs with non-migrated messages.`,
  );
  ...
  for (const item of messagesWithNoConv) {
    ...
    try {
      ...
      console.log(
        `Migrated ${result.count} messages for Job ${job.id} to Conversation ${conversation.id}`,
      );
    } catch (error) {
...
async function getOrCreateConversation(clientId: number, freelancerId: number) {
  ...
  if (!conversation) {
    console.log(
      `Creating new conversation: Client ${clientId} <-> Freelancer ${freelancerId}`,
    );
