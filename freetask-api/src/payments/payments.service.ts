...
export class PaymentsService {
    ...
    async createPayment(dto: CreatePaymentDto) {
        ...
        this.logger.log(`Preparing Billplz for Job #${job.id}, Amount: ${amountInCents} cents`);
    ...
    async checkAndCompletePayment(billId: string) {
        ...
        if (bill && bill.state === 'paid') {
            this.logger.log(`Job Payment ${billId} verified as PAID via API check. Completing payment...`);
            await this.markPaymentAsCompleted(payment);
            // Refresh payment
    ...
    private async markPaymentAsCompleted(payment: any) {
        this.logger.log(`Completing payment ${payment.id} for Job ${payment.jobId}`);
        ...
        await this.prisma.$transaction(async (tx) => {
            ...
            this.logger.log(`Payment completed, job ${payment.jobId} moved to IN_PROGRESS, escrow held`);
        });
    ...
    async retryPayment(jobId: number) {
        ...
        this.logger.log(`Payment retry created for job ${jobId}, new bill: ${billplzResponse.id}`);
    ...
    async processJobPayout(jobId: number) {
        ...
        if (updateResult.count === 0) {
            this.logger.warn(`Skipping payout for Job #${jobId}: Invalid state or already processing.`);
            return { status: 'skipped', message: 'Job not in valid state for payout' };
        }
        ...
        if (!job || !job.freelancer.bankCode || !job.freelancer.bankAccount) {
            ...
            this.logger.error(`Missing bank details for Job #${jobId}. Moving to PAYOUT_FAILED.`);
        ...
        if (job.freelancer.trustScore < TRUST_THRESHOLD) {
            this.logger.warn(`Job #${jobId} payout held: Trust Score ${job.freelancer.trustScore} < ${TRUST_THRESHOLD}`);
            await this.prisma.job.update({
                where: { id: jobId },
                data: {
                    status: 'PAYOUT_HOLD',
        ...
        try {
            // 2. CALL BILLPLZ API
            const amountInCents = Math.round(Number(job.freelancerPayoutAmount) * 100);
    ...
    async processStuckPayouts() {
        ...
        for (const job of stuckJobs) {
            ...
            if (job.payoutRetryCount >= 3) {
                this.logger.error(`Job #${job.id} exceeded retry limit. Moving to PAYOUT_FAILED_MANUAL.`);
                await this.prisma.job.update({
                    where: { id: job.id },
            ...
            this.logger.log(`Retrying stuck payout for Job #${job.id} (Attempt ${job.payoutRetryCount + 1})`);
